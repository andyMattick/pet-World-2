...
| File | What must stay |
|---|---|
| `src/game/game.js` | About 1,280 lines. The `GEN` object with 14 generators: `basic, tape, groups, dnlCreate, dnl, dnlTable, table, equiv, word, realworld, understand, coord, units, ppw`. Functions `submit`, `completeOrder`, `openPractice`, `enterAs`, `openJoin`, `buyReward`, `tableDown`, `simplestStep`, `simplestChoice`, `renderHall`, `renderParent`; Fact Sprint and drill mixing stay intact. |
| `src/shared/registry.ts` | Exports `SKILLS`, `SKILL_ORDER`, `STATIONS`, `UNLOCK_AT`, `MIS`, `notSimplest`, `statusFromRecent`, `DRILLS`, `drillLabel`, `DrillSettings`, `DEFAULT_DRILL_SETTINGS`, `mergeDrillSettings`, `drillTypeOn`. |
