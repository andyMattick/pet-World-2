...
| `src/shared/registry.ts` | Exports `SKILLS`, `SKILL_ORDER`, `STATIONS`, `BAKERY_STATIONS`, `SHOPS`, `shopOfSkill`, `UNLOCK_AT`, `MIS`, `notSimplest`, `statusFromRecent`, `DRILLS`, `drillLabel`, `DrillSettings`, `DEFAULT_DRILL_SETTINGS`, `mergeDrillSettings`, `drillTypeOn`. |
