-- Teacher controls for practice pop-ups (drills): class defaults and per-student overrides.
-- Run AFTER 20260925000000_drills.sql.
alter table public.classes  add column if not exists drill_settings jsonb not null default '{}'::jsonb;
alter table public.students add column if not exists drill_settings jsonb;
alter table public.classes  add constraint classes_drill_settings_object  check (jsonb_typeof(drill_settings) = 'object');
alter table public.students add constraint students_drill_settings_object check (drill_settings is null or jsonb_typeof(drill_settings) = 'object');
-- Teachers already update classes and students through existing row-level security policies.

-- my_student(): unchanged, plus the class and student drill settings
...
-- class_report(): unchanged, plus 'ds' (the student's drill override) and 'dl' (drill history from the saved town)
